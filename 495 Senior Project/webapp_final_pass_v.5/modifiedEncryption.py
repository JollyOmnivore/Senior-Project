...
validVote = ['a', 'b', 'c', 'd', 'e']	# list of valid input options for user vote
...
while(votConf != "y"): # checks against user input for confirming vote
    ...
    print("Option C: Jerky		Option D: Coffee")
    print("Option E: Abstain")
    usrVote = input("Please select your vote: A, B, C, D, E:     ")
...
for i in range(0, voterCount):
    print(str(i) + " is current index ========")
    decryptedTempVal = decryptTotal(voteArray[i], lam, n, mu)
    print("decrypted val: " + str(voteArray[i]) + " to vote: " + str(decryptedTempVal))
    #actualResult = decryptedTempVal + actualResult
...
print("No votes/abstains: " + str(totalNoVotes))
